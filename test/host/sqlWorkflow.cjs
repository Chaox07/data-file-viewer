...
(async () => {
  ...
  try {
    ...
    await frame.locator('#query-schema').filter({ hasText: 'Date: VARCHAR' }).waitFor();
    ...
    console.log('Installed-host SQL workflow passed: types, draft, handoff, restore, exact date boundary, 100 rows and unchanged source.');
  } finally {
