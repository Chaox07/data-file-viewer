...
(async () => {
  ...
  try {
    ...
    await page.waitForFunction(() => document.querySelector('td[data-r="3"][data-c="1"]')?.style.boxShadow.includes('inset'));
    assert.equal(await page.locator('.row-num').first().textContent(), '1');
    assert.equal(await page.locator('#query-schema').count(), 0);
    assert.equal(await page.locator('#query-preview').count(), 0);
    const detailsDeadline = Date.now() + 10000;
    while (!responses.some(m => m.command === 'queryTargetDetails' && m.target.id === main.value) && Date.now() < detailsDeadline) await page.waitForTimeout(20);
    const details = responses.find(m => m.command === 'queryTargetDetails' && m.target.id === main.value);
    assert.ok(details?.columns.some(c => c.name === 'Date' && c.type === 'VARCHAR'));
    assert.equal(await page.locator('td[data-r="0"][data-c="1"]').evaluate(td => td.style.boxShadow), '');
    assert.equal(await page.evaluate(() => sqlTest.inspectEditor()), 'SELECT 42 AS saved_draft;');
    await editor.fill('SELECT * FROM "memory"."main"."Raw_Data · Table 2" t WHERE t.Da');
    await editor.press('End');
    await editor.press('Control+Space');
    await page.locator('.cm-completionLabel').filter({ hasText: /^Date$/ }).waitFor();
